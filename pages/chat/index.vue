    ...
    >
      ...
      <UForm :state="state" @submit="onSubmit" class="space-y-6">
        ...
        <UButton
          ...
          size="xl"
          block
          :disabled="!state.username.trim() || !state.room"
        >
          Join Discord
        </UButton>
      </UForm>
...
const rooms = ["Netflix Room", "Dorama (ドラマ)", "Anime Room", "K-Drama"];
...
async function onSubmit(event: FormSubmitEvent<any>) {
  navigateTo(`/chat/room?username=${state.username}&room=${state.room}`);
}
</script>
